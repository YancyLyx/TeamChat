...
Dashboard 采用 Slack/Discord 风格的聊天室界面。人类在输入框打字，agent 以聊天消息形式呈现活动。消息路由由 Engine 自动处理。
...
│  状态     │       📌 主聊天区              │              │
│          │                                │ 📝待处理 (2) │
│ 🏗️cici咪 │  🏗️ cici咪: 创建了 Issue #7   │ 🔧进行中 (1) │
│  🟢 空闲   │  ⚡ coco咪: 领取了 Issue #7    │ ✅已完成 (5) │
│          │  🔍 soso咪: PR #5 Review ✅    │              │
│ ⚡coco咪 │                                │              │
│  🔴 忙碌   ├────────────────────────────────┤              │
│          │  💬 @cici咪 分析一下性能...     │              │
...
| 人类输入 | 路由结果 |
|---|---|
| `@coco咪 修复 Dark Mode` | → coco咪 直接执行 |
| `@soso咪 跑一下测试` | → soso咪 直接执行 |
| `Dashboard 加刷新按钮` | → cici咪 分析 → @coco咪 |
| `项目性能优化一下` | → cici咪 拆任务 → 分给多 agent |
| `hello，在吗？` | → cici咪 回复团队状态 |
