...
## cici咪 任务审查闭环（#97，设计 2026-08-05 v2）

**状态**: Draft（v2 按用户真实流程重写：done 先行 + 审查节点后置）

---

### 背景与动机

现状缺陷：cici咪 任务按 exit_code 自动收尾（无独立验证）。用户真实流程（人肉路由验证可行）：cici咪 开发完 → 写审查 prompt 给 soso咪 → 审查+测试 → 结果回 cici咪 → 判断：无问题收尾；有问题继续修复 → 修复后再审 → 循环。

### 目标（v2 核心模型）

> **每个节点的"审查"是一个正式 DAG 节点**（soso咪 执行，继承 feature_id）：
> 节点完成 → cici咪 初步审核 → 标 done（解锁下游）→ **创建审查节点**（soso咪）
> → 审查节点完成 → cici咪 审核审查结论 → 无问题标审查 done；
> 有问题 → **创建修复节点**（继承 feature_id）→ 修复完成 → **再建复审节点** → 循环直到通过。

### 完整流程（用户举例，2026-08-05）

```
需求"实现看板" → cici咪 分析 → 构建 DAG → 写 A/B 节点 prompt → 引擎派发

A(cici咪 引擎开发)  ──→ C2(soso咪 审查A) ──→ D(cici咪 修复C2问题) ──→ E(soso咪 复审修复)
B(coco咪 前端开发)  ──→ C1(soso咪 审查B)

时序（含并行/排队）：
1. A、B 并行执行（不同 agent，#96）
2. B 先完成 → 结果回流排队（cici咪 做 A 中，忙）→ 等 cici咪 空闲
3. cici咪 审 B → 通过 → B 标 done → 写审查 prompt → 建 C1(soso咪 审查 B)
4. A 完成 → cici咪 审 A → A 标 done → 建 C2(soso咪 审查 A)；C2 排队（soso咪 做 C1）
5. C1 完成 → cici咪 审审查结论 → 无需修改 → C1 标 done
6. soso咪 空闲 → 执行 C2 → 完成 → cici咪 审结论：两处要改
   → 建 D(cici咪 修复，继承 feature_id) → 派发
7. D 完成 → cici咪 审 D → D 标 done → 建 E(soso咪 复审修复)
8. E 完成 → cici咪 审 → 通过 → E 标 done → 需求完成
```

### 关键设计点

1. **TaskScheduler**：删除 cici咪 自动收尾 → 所有任务统一 relay（cici咪 审核）
2. **ResultRelay**：cici咪 任务结果排队进审核（不再跳过）；审核 prompt 区分两种模式：
   - 普通模式（cici咪 审 coco咪/soso咪 结果，含审查节点结论）：对照 AC 验收 → 标 done
   - **自我编排模式**（cici咪 审自己的执行结果）：引导——"标记 done 后请创建 soso咪 审查任务（create_task，prompt 写明审查点）"——**审查节点是后续 DAG 节点，不是验收闸门**
3. **审查节点 = 普通 DAG 节点**（agent=soso咪，feature_id 继承）：创建、依赖、排队、并行、回流全部复用现有机制（#96 并行派发天然支持）
4. **修复节点**：审查结论发现问题 → cici咪 创建修复任务（通常自己修，继承 feature_id）→ 修复完成 → 复审节点（循环）
5. **cici咪 仍不直接验收自己**（铁律精神）：她的开发节点 done 后，审查节点由引擎流程引导必须创建（prompt 强引导 + 看板可见审查节点存在性）
6. 引擎仍不直接标 done/failed——收尾由 cici咪 审核执行（审查结论也是她审核的对象）

### 与 v1 的差异

| | v1（审查闸门） | v2（用户流程，done 先行 + 审查后置） |
|---|---|---|
| 节点 done 时机 | 审查通过后才 done | cici咪 初步审核后即 done |
| 审查角色 | 验收闸门（无状态） | 正式 DAG 节点（可排队/并行/统计） |
| 修复 | 节点保持 running | 追加修复节点 + 复审节点（循环） |
| 下游解锁 | 慢（等审查） | 快（done 即解锁，审查并行） |

### 边界与风险

- **审查节点成本**：每个开发节点 +1 个 soso咪 审查节点（多一轮往返）——换来独立验证；用户流程已验证接受
- **滥用风险**：cici咪 可能不建审查节点直接收尾——prompt 强引导 + Stats 可观测（需求树的审查节点覆盖率）
- **卡死风险**：审查节点卡住 → 下游 pending——与现有链路同构，看板手动干预兜底
- **审查节点依赖**：C1 依赖 B done（B 先收尾再审查）——审查节点创建时带 depends_on=[被审查节点]（语义：审查在节点完成后进行）
- **修复后复审**：E 复审 D 的修复——修复节点 done 后同样引导创建复审节点

### 测试计划

| 测试 | 内容 |
|---|---|
| 单测：不自动收尾 | cici咪 任务完成后 status 保持 running（等待 cici咪 审核，不自动 update done） |
| 单测：relay 排队 | cici咪 任务结果进审核队列（不再跳过） |
| 单测：审核模式 | 自我编排模式 prompt 含"创建 soso咪 审查任务"引导；普通模式不含 |
| 集成：完整链路 | A 完成 → cici咪 审核标 done → 建 C2(soso咪) → C2 完成回流 → cici咪 审核标 done；审查发现问题 → D 修复 → E 复审循环 |
| 回归 | coco咪/soso咪 任务审核链路不变（现有测试全绿） |
| 真实 e2e | 按用户例子全流程走一遍（A/B 并行 → C1/C2 排队 → 修复 D → 复审 E） |

### 验收标准

1. cici咪 开发任务完成后**不自动 done**（单测断言）
2. 自我编排模式引导创建 soso咪 审查节点；审查节点正常执行、排队、回流
3. 审查结论发现问题 → 修复节点 → 复审节点循环可重复
4. coco咪/soso咪 任务审核链路完全不变（回归）
5. 全量套件 + 真实 e2e（用户例子全流程）通过
