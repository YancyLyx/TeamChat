...
## cici咪 任务审查闭环（#97，设计 2026-08-05 v3）

**状态**: Draft（v3 = 用户确认的完整流程；开发按此设计，变更需更新本节）

---

### 背景与动机

现状缺陷：cici咪 任务按 exit_code 自动收尾（无独立验证）。目标：把用户真实流程（cici咪 写审查 prompt → soso咪 审查测试 → 结果回 cici咪 判断 → 修复 → 复审循环）自动化。

### 核心模型（v3，用户确认）

> **审查节点是正式 DAG 节点**（soso咪 执行，feature_id 继承，可排队/并行/统计）。
> **初始 DAG 就包含 soso咪 的验证节点**（三只咪都在图里）——被审节点 done 后审查节点自动解锁派发，无需事后手动创建。
> 审查 prompt 初始可写（审查点 + AC 对照）——soso咪 执行时自己 git diff / 读代码 / 跑测试。
> 修复节点与复审节点在审查发现问题后由 cici咪 创建（prompt 需要时写）。

### 完整流程（需求"实现任务看板"）

**① 拆任务（cici咪 分析后，MCP create_task 一次建全）**：

```
A [cici咪] 引擎开发: task_table stats API
B [coco咪] 前端开发: 看板 UI
C1 [soso咪] 审查B: 对照 B 的 AC 审查代码改动 + 跑测试      （depends_on=[B]）
C2 [soso咪] 审查A: 对照 A 的 AC 审查代码改动 + 跑测试      （depends_on=[A]）
```

**② 并行派发**：A、B 无依赖、agent 空闲 → 并行派发（#96）→ A、B 同时 running。

**③ B 先完成 → 排队等审核**：B 结果回流 → relay 排队（cici咪 做 A 中，忙）→ 等 cici咪 空闲。

**④ cici咪 审 B → B done → C1 自动解锁**：cici咪 空闲，队列先出 B（FIFO）→ 普通审核（对照 B 的 AC）→ 通过 → **B 标 done** → C1（depends_on B）自动解锁 → 引擎派发给 soso咪。

**⑤ A 完成 → cici咪 审 A → A done → C2 自动解锁**：A 结果回流（cici咪 空闲）→ 自我编排模式审核（自己的执行结果，引导确认"审查节点 C2 已就绪"）→ A 标 done → C2 解锁，但 **soso咪 正在做 C1** → C2 排队。

**⑥ C1 完成 → cici咪 审审查结论**：C1（soso咪 审 B）结果回流 → cici咪 普通审核（审的是 soso咪 的审查结论）→ 无需修改 → **C1 标 done**。

**⑦ C2 执行 → 发现问题 → 建修复节点**：soso咪 空闲 → 执行 C2 → 回流 → cici咪 审结论 → **发现两处要改** → cici咪 建 D：`D [cici咪] 修复 C2 发现的两处问题`（继承 feature_id，prompt 需要时写，写明两处问题）→ 引擎派发。

**⑧ D 完成 → cici咪 审 D → D done → 建复审节点**：D 结果回流 → cici咪 自我编排审核 → D 标 done → 建 E：`E [soso咪] 复审 D 的修复`（depends_on=[D]）→ 派发。

**⑨ E 完成 → cici咪 审结论 → 通过 → E done** → 需求树全部 done，需求完成。

### 贯穿规则

1. **引擎从不直接标 done/failed**——所有收尾由 cici咪 审核后执行（MCP update_task）
2. **审核双模式**：
   - 普通模式（cici咪 审 coco咪/soso咪 的结果，含审查节点结论）：对照 AC 验收 → 标 done / 追加修复
   - **自我编排模式**（cici咪 审自己的执行结果）：确认对应审查节点已就绪（初始已建），不直接收尾了事
3. **审查节点初始创建**（拆任务时建全）——被审节点 done 自动解锁派发；审查 prompt 初始写（审查点 + AC），soso咪 执行时自己看代码/跑测试
4. **修复/复审节点动态创建**（审查发现问题后）——prompt 需要时写（基于审查结论）
5. **排队**：cici咪 忙时（③）与 soso咪 忙时（⑤）——复用现有 relay 排队 + 同 agent 串行

### 与 v2 的差异

| | v2（审查事后建） | v3（用户确认，审查初始建） |
|---|---|---|
| 初始 DAG | 只有开发节点 | 含审查节点（三只咪在图里） |
| 审查触发 | cici咪 事后手动建 | 被审节点 done 自动解锁派发 |
| 审查 prompt | 事后写（基于产出） | 初始写（审查点+AC；soso咪 自己看代码） |

### 边界与风险

- **审查成本**：每开发节点 +1 审查节点（一轮 soso咪 往返）——用户流程已验证接受
- **滥用/遗漏**：cici咪 可能不建审查节点（初始建全后不存在此问题）；修复节点后是否建复审由 cici咪 判断，prompt 强引导 + 看板可见
- **卡死**：审查节点卡住 → 下游 pending——与现有链路同构，看板手动干预兜底
- **审查节点依赖**：C1 依赖 B（B done 才审查）——审查不与被审节点并行抢 soso咪
- **git 安全**：soso咪 审查时可能执行 git 命令（git diff）——审查节点 prompt 允许只读 git 命令（diff/log/status），禁止 checkout/reset/commit 等写操作
...
| 单测：审核双模式 | 自我编排模式 prompt 含"审查节点已就绪"引导；普通模式不含 |
| 集成：初始 DAG | A/B/C1/C2 一次建全；B done → C1 自动解锁派发（无需手动建） |
| 集成：修复复审循环 | C2 发现问题 → D 修复 → E 复审 → E done |
| 回归 | coco咪/soso咪 任务审核链路不变（现有测试全绿） |
| 真实 e2e | 按上述 9 步全流程走一遍 |

### 验收标准

1. cici咪 开发任务完成后不自动 done（单测断言）
2. 初始 DAG 含审查节点；被审节点 done 后审查节点自动解锁派发（单测断言）
